...
use crate::{bindings::*, notify::*, utils::assert_send_sync};
...
#[windows_core::implement(IMarkdownContent, IContent, INotifyPropChanged)]
...
impl MarkdownContent {
    ...
    pub fn new(body: impl Into<HSTRING>) -> ComObject<Self> {
        Self::new_unmanaged(body).into()
    }
}

impl MarkdownContent_Impl {
    fn emit_self_prop_changed(&self, prop: &str) {
        let sender = self.to_interface::<IInspectable>();
        let arg = IPropChangedEventArgs::from(PropChangedEventArgs(prop.into()));
    ...
    pub fn body(&self) -> Result<NotifyLockReadGuard<'_, HSTRING>> {
        self.body.read()
    }
    ...
    pub fn body_mut(&self) -> Result<NotifyLockWriteGuard<'_, HSTRING>> {
